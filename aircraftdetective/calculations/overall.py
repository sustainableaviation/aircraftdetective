...
pol1, pol2, _, _ = engines.determine_takeoff_to_cruise_tsfc_ratio(
    path_excel_engine_data_for_calibration=path_engine_tsfc_database
)
...
df_merged = pd.merge(
    left=df_aircraft,
    right=df_concordance,
    how='left',
    on='Aircraft Designation',
)

df_merged = pd.merge(
    left=df_merged,
    right=df_aircraft_aggregated,
    how='left',
    on='Aircraft Designation (aircraft-database.com)',
).drop(columns=['Aircraft Designation (aircraft-database.com)'])